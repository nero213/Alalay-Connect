...
const filteredConversations = computed(() => {
  ...
  return conversations.value.filter((conv) => conv.other_person_name?.toLowerCase().includes(query))
})
...
const isImageFile = (fileType, fileName) => {
  ...
  return (
    fileType?.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext)
  )
}
...
const markAsRead = async () => {
  ...
  try {
    ...
    const conv = conversations.value.find(
      (c) => c.conversation_id === activeConversation.value.conversation_id,
    )
  ...
  try {
    ...
    if (response.conversation_id && !activeConversation.value) {
      ...
      const newConv = conversations.value.find(
        (c) => c.conversation_id === response.conversation_id,
      )
      if (newConv) {
    ...
    if (activeConversation.value) {
      const lastMessage = previewFile.value
        ? isImageFile(previewFile.value.type, previewFile.value.name)
          ? '📷 Image'
      ...
      >
        ...
        <div class="conversations-list">
          ...
          <div v-else-if="filteredConversations.length === 0" class="empty-state">
            ...
            <router-link v-if="!searchQuery" to="/dashboard" class="browse-link"
              >Browse Professionals</router-link
            >
          </div>
            ...
            >
              ...
              <div class="conversation-info">
                ...
                <span class="conversation-type">{{
                  conv.other_person_type === 'skilled' ? 'Skilled Worker' : 'Client'
                }}</span>
              </div>
            </div>
          </div>
        ...
        <div v-else class="chat-container">
          ...
          <div class="chat-header">
            ...
            <div class="chat-header-info">
              <img
                :src="getAvatarUrl(activeConversation.other_person_image)"
                :alt="activeConversation.other_person_name"
                class="chat-avatar"
              />
              <div>
            ...
            <div v-else>
              <div
                ...
                :class="[
                  'message-item',
                  {
                    'message-sent': msg.sender_id === currentUser?.id,
                    'message-received': msg.sender_id !== currentUser?.id,
                  },
                ]"
              >
                <div class="message-bubble">
                  ...
                  <div
                    v-if="msg.file_url && isImageFile(msg.file_type, msg.file_name)"
                  ...
                  <div v-else-if="msg.file_url" class="file-attachment">
                    <a :href="getFileUrl(msg.file_url)" target="_blank" class="file-link">
                      <span class="file-icon">{{ getFileIcon(msg.file_type, msg.file_name) }}</span>
                      <div class="file-info">
                  ...
                  <div class="message-footer">
                    ...
                    <span
                      v-if="msg.sender_id === currentUser?.id && msg.is_read"
                      class="read-status"
                    >
                      <svg
                        width="14"
