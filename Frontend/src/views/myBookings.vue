    ...
    <div class="my-bookings-page">
        ...
        <div class="bookings-container">
            ...
            <div class="filter-tabs">
                <button @click="statusFilter = 'all'" :class="['tab-btn', { active: statusFilter === 'all' }]">
                    All ({{ counts.all }})
                </button>
                <button @click="statusFilter = 'pending'" :class="['tab-btn', { active: statusFilter === 'pending' }]">
                ...
                <div v-for="booking in filteredBookings" :key="booking.booking_id" class="booking-card">
                    <div class="booking-header">
                        <div class="professional-info">
                            <img :src="getImageUrl(booking.skilled_image)" :alt="booking.skilled_firstName"
                    ...
                    <div class="booking-details">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value amount">₱{{ booking.total_amount?.toLocaleString() || 'TBD'
                            }}</span>
                        </div>
                    </div>

                    <div class="booking-actions">
                        <button @click="viewDetails(booking.booking_id)" class="btn-view">
                            View Details
                        </button>
                        <button v-if="booking.status === 'pending' || booking.status === 'confirmed'"
                            @click="openCancelModal(booking)" class="btn-cancel">
                            Cancel Booking
                        </button>
                        <button v-if="booking.status === 'confirmed'" @click="openRescheduleModal(booking)"
                            class="btn-reschedule">
                            Reschedule
                        </button>
                        <button v-if="booking.status === 'completed' && !hasUserRated(booking)"
                            @click="leaveReview(booking)" class="btn-review">
                            Leave Review
                        </button>
                    </div>
                </div>
            ...
            <div v-else class="empty-state">
                ...
                <p>You don't have any {{ statusFilter !== 'all' ? statusFilter : '' }} bookings at the moment.</p>
        ...
        <div v-if="showCancelModal" class="modal-overlay" @click="closeModals">
            <div class="modal-content" @click.stop>
                ...
                <div class="modal-body">
                    <p>Are you sure you want to cancel this booking with <strong>{{ selectedBooking?.skilled_firstName
                    }} {{ selectedBooking?.skilled_lastName }}</strong>?</p>
                    <div class="form-group">
                        <label>Reason for cancellation <span class="required">*</span></label>
                        <textarea v-model="cancelReason" placeholder="Please provide a reason for cancellation..."
                ...
                <div class="modal-footer">
                    ...
                    <button @click="confirmCancel" class="btn-danger" :disabled="!cancelReason.trim() || cancelling">
            ...
            <div class="modal-content" @click.stop>
                ...
                <div class="modal-body">
                    <div class="current-booking">
                        <p><strong>Current Date:</strong> {{ formatDateTime(selectedBooking?.service_date) }}</p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea v-model="rescheduleForm.reason"
                            placeholder="Let the professional know why you need to reschedule..." rows="2"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button @click="closeModals" class="btn-secondary">Cancel</button>
                    <button @click="confirmReschedule" class="btn-primary"
...
import { getClientBookings, cancelBooking, rescheduleBooking, getAvailableSlots } from '@/api/bookingService'
...
const loadAvailableSlots = async () => {
    ...
    try {
        const response = await getAvailableSlots(selectedBooking.value.skilled_id, rescheduleForm.value.date)
